...
function createCall(target, display_name, media_stream, logger, api_host) {
    if (!logger) {
    ...
    return new Promise((resolve, reject) => {
        ...
        fetch(api_url, {
            ...
            let call = call_1.Call(cfg, logger, false, media_stream);
            resolve(call);
        })
            .catch((error) => {
            reject(error);
...
(function (ContentState) {
    ContentState[ContentState["SEND"] = 0] = "SEND";
    ContentState[ContentState["RECV"] = 1] = "RECV";
    ContentState[ContentState["DISABLED"] = 2] = "DISABLED";
