...
function Call(config_, base_logger, logSdp) {
    var config = __assign({}, config_, { plugin: false, allowH264: sl.detectedBrowser === 'safari' });
    ...
    var dial = function (options) {
        ...
        if (config.target && (options.mediaConstraints || options.mediaStream)) {
            userAgent = new JsSIP.UA(__assign({}, config, { sockets: [new JsSIP.WebSocketInterface(config.websocket_address)] }));
    ...
    var callConnected = function (target, options) {
        ...
        logger.info('Sending sip invite to ', target, ' with options ', options.rtcOfferConstraints);
