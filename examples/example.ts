import { createCall, SlCall, CallEndReason, detectedBrowser, detectedVersion, VolumeMeter, Logger } from "../lib/index";
...
window.onload = function() {
    ...
    start_call_button.addEventListener("click", () => {
        ...
        createCall(target, "Example WebRTC client", local_stream, logger, cloud_addr)
            .then((call: SlCall) => {
                call.on("remote_video", (remote_stream: MediaStream | undefined) => {
                    logger.debug("SlCall::remote_video", remote_stream);
                    if (remote_stream !== undefined) {
                        remote_video.srcObject = remote_stream;
                    }
                });
                call.on("remote_content", (remote_stream: MediaStream | undefined) => {
                    logger.debug("SlCall::remote_content", remote_stream);
                    if (remote_stream !== undefined) {
