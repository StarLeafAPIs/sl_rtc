...
import { ILogger, getBrowser } from '../sl';
import { PCState, media_type } from './constants';
...
export function SdpMunger(
    ...
    let local_media: { [k in media_type]: boolean } = {
        audio: false,
        video: false
    };
    ...
    let mungeLocal = function(data: SdpData) {
        ...
        session.media.forEach(function(
            mline: { type: media_type; [key: string]: any },
            index: number
        ) {
            if (mline.port == 0) {
                if (mline.rtcpMux) {
