...
import { ContentState, CallEndReason, MuteState, SlCall, CallEventMap, CallConfig } from "./interface";
...
export function Call(config_: CallConfig, base_logger: ILogger, logSdp: boolean, media_stream?: MediaStream): SlCall {
    let config = {
        ...config_,
        websocket_port: 443
    };
    let logger = base_logger.sub("CALL");
    let stats_manager = StatsManager(statsPeriod, logger);
    ...
    let handlers = (function() {
        let reg: CallEventHandlers = {
            ...
            remote_video: { cb: null, oneshot: false }, // remote MediaStream object
            remote_content: { cb: null, oneshot: false }, // remote MediaStream object
    ...
    let user_agent: JsSIP.UA;
    let rtc_session: JsSIP.RtcSession;
    let callEnding = true;

    let await_ice_reinvite = true;
    let connection_timeout_timer: number = -1; // If we don't connect a websocket within 10 seconds, fail the call

    let sdp_munger = SdpMunger(logger, logSdp);
    let audio_only_call = false;
    let connected_ws = false;
    let mute_status: MuteState;

    let local_av_stream = media_stream;
    let video_transceiver: any;

    let local_content_stream: MediaStream | undefined;
    let content_transceiver: any;
    let content_retry_timer: number = -1;
    ...
    function dial() {
        ...
        user_agent.on("newRTCSession", e => {
            rtc_session = e.session as JsSIP.RtcSession;
            mute_status = isMuted();
            ...
            rtc_session.on("peerconnection", (event: { peerconnection: RTCPeerConnection }) => {
    ...
    function translateSipEvent(event: any) {
        let r = CallEndReason;
    ...
    function onCallError(jssipEvent?: string) {
        shutdown();
    ...
    function userAgentDisconnect(event: any) {
        shutdown();
    ...
    function mute(set_mute: MuteState): boolean {
        if (rtc_session && local_av_stream) {
            ...
            if (mute_status.video !== mute_status_old.video) {
                rtc_session.sendInfo(
                    "application/media_control+xml",
                    "<stream_id>video</stream_id>" + "<mute>" + (mute_status.video ? 1 : 0) + "</mute>"
                );
            }
            return true;
        } else {
    ...
    function onIceChange(event: Event) {
    ...
    function setupPeerConnection(pc: any) {
        ...
        pc.oniceconnectionstatechange = function(ev: any) {
            onIceChange(ev);
        };
    ...
    function startContent(stream: MediaStream) {
        ...
        let peerConnection = rtc_session.connection as any;
    ...
    function onSdp(data: SdpData) {
        ...
        if (data.originator === "local") {
            ...
            if (data.type === "offer") {
                if (content_state === ContentState.RECV && content_transceiver && local_content_stream) {
                    content_transceiver.direction = "recvonly";
                    local_content_stream.getVideoTracks()[0].stop();
                    local_content_stream = undefined;
                    logger.debug("Switching transceiver to remote content");
                    handlers.notify("local_content", undefined);
                } else if (content_state === ContentState.SEND && !local_content_stream) {
                    logger.error("StarLeaf offered to receive content when WebRTC is not sending");
                    sdp_munger.disableContent(data);
                } else if (content_state === ContentState.DISABLED && local_content_stream) {
                ...
                if (content_state === ContentState.DISABLED && local_content_stream) {
